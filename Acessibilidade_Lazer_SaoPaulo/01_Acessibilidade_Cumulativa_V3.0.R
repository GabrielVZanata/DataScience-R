...
######################## JAN 2023  ########################
  ...
  ### Define o vetor com os PuT (tempos mínimos de acesso) desejados
  ### ATENÇÃO: Quanto maior o vetor, maior o tempo de execução
    PuT <- c(30,45,60,90,120)  
      
    ### Leitura do arquivo shapefile da area construida da RMSP
    ...
    d_1 <- read.csv("PT_Time_SP_v3_pt1.csv", header=FALSE, sep = ";", dec = ",")
    d_2 <- read.csv("PT_Time_SP_v3_pt2.csv", header=FALSE, sep = ";", dec = ",")
    d_3 <- read.csv("PT_Time_SP_v3_pt3.csv", header=FALSE, sep = ";", dec = ",")
    d_4 <- read.csv("PT_Time_SP_v3_pt4.csv", header=FALSE, sep = ";", dec = ",")
    d_5 <- read.csv("PT_Time_SP_v3_pt5.csv", header=FALSE, sep = ";", dec = ",")
    d_6 <- read.csv("PT_Time_SP_v3_pt6.csv", header=FALSE, sep = ";", dec = ",")
    
    dij2_p <- cbind(d_1,d_2,d_3,d_4,d_5,d_6)
    
    ### Remove a linha e a coluna 1317 da matriz de tempos, pois não existe essa zona no arquivo shapefile
    dij2_p <- dij2_p[-1317, -c(1317)]
  
  ########### LOOP PARA OS DIFERENTES PuTs ########################
    ...
    for (i in PuT){  
      ...
      ### Criação do novo arquivo shapefile com acessibilidades
      ...
      shapefile(combine, paste("shapefiles/Acessibilidade_Cumulativa_PuT_", i,"min.shp", sep=""), overwrite=TRUE)
      map <- readOGR(dsn="shapefiles",layer= paste("Acessibilidade_Cumulativa_PuT_", i,"min", sep=""))
      ...
      filename <- paste("plots/CumAcc_Put",i, ".pdf",sep = "")
      pdf(file = paste("plots/CumAcc_Put",i, ".pdf",sep = ""))
      # png(filename = paste("plots/CumAcc_Put",i, ".png",sep = ""), width = 1000, height = 1000, units = "px")
